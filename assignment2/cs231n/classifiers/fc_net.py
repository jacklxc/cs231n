...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    scores, cache2 = affine_relu_forward(h,self.params["W2"],self.params["b2"])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dh, grads['W2'], grads['b2'] = affine_relu_backward(dloss, cache2)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(self.num_layers-1):
        if self.use_dropout:
            if self.use_batchnorm:
                h, cache = affine_batchnorm_relu_forward(h,self.params["W"+str(i+1)],\
    ...
    dh = dloss
    # Deal with the last iteration
    dh, grads['W'+str(self.num_layers)], grads['b'+str(self.num_layers)] = affine_backward(dh, caches[self.num_layers-1])
    grads['W'+str(self.num_layers)] += self.reg * self.params['W'+str(self.num_layers)]
    # Then the rest of the iteration
    for i in reversed(xrange(self.num_layers-1)):
        if self.use_dropout:
            ...
            if self.use_batchnorm:
                dh, grads['W'+str(i+1)], grads['b'+str(i+1)], grads['gamma'+str(i+1)], grads['beta'+str(i+1)]\
                 = affine_batchnorm_relu_backward(dh, cache)
            ...
            if self.use_batchnorm:
                dh, grads['W'+str(i+1)], grads['b'+str(i+1)], grads['gamma'+str(i+1)], grads['beta'+str(i+1)]\
                 = affine_batchnorm_relu_backward(dh, caches[i])
                grads['W'+str(i+1)] += self.reg * self.params['W'+str(i+1)]
            else:
               dh, grads['W'+str(i+1)], grads['b'+str(i+1)] = affine_relu_backward(dh, caches[i])
               grads['W'+str(i+1)] += self.reg * self.params['W'+str(i+1)]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
